...
- **Especificación técnica del Listado de Fauna CITES Perú (2018):**
  ...
  - Aclaración sobre las 16 especies registradas en el Apéndice III (no
    contabilizadas en el total oficial nacional por no haber solicitado
    el Perú inclusiones en dicho apéndice).
...
- **Especificación técnica del Listado de Flora CITES Perú (2018):**
  ...
  - Documentación del sistema de 24 acrónimos departamentales de Lamas &
    Encarnación (1976), el significado del símbolo `?` y las anotaciones
    de derivados (`#`).
  - Registro de herbarios físicos (USM, MOL) y virtuales (MO, US, NY,
    F).
- **Incorporación y exportación de datos (`data/`):**
  - Se procesaron y exportaron mediante `usethis::use_data()` los cuatro
    listados oficiales CITES de Perú:
    - `cites_fauna_peru_2018` (512 observaciones, 17 variables: 496
      especies oficiales reguladas + 16 de Apéndice III).
    ...
    - `cites_fauna_peru_2023` (568 observaciones, 17 variables:
      resoluciones CoP19 Panamá 2022 vigentes en 2023, competencia
      sectorial SERFOR/PRODUCE y año de inclusión/enmienda).
    - `cites_flora_peru_2018` (2506 observaciones, 10 variables: listado
      completo en 9 familias botánicas nativas).
    - `codigos_departamentos_pe` (24 observaciones, 3 variables:
      acrónimos estándar de Lamas & Encarnación 1976 y códigos UBIGEO
      del INEI).
  ...
  - Se documentaron exhaustivamente las variables, fuentes oficiales y
    ejemplos de uso en `R/data.R` y se compilaron los manuales
...
- **Sistema de consulta y concordancia taxonómica (matching):**
  - Se implementó el motor de matching taxonómico inspirado en la
    ...
    - [`cites_match()`](https://paulesantos.github.io/citesperu/reference/cites_match.md)
      ...
      tubería secuencial optimizada con direct match (coincidencia
      exacta), synonym match (resolución de sinónimos oficiales a taxón
      CITES aceptado y su Apéndice), suffix match (flexión de género en
      sufijos latinos), fuzzy match acotado al género por distancia de
      edición (`max_dist`), y genus match para taxones regulados a nivel
      genérico o con calificador `sp.`/`spp.`.
    - [`is_cites()`](https://paulesantos.github.io/citesperu/reference/is_cites.md)
      ...
      (`TRUE`/`FALSE`/`NA`).
  - Se compiló el backbone interno pre-indexado en `R/sysdata.rda`
    integrando 5,816 registros taxonómicos (3,053 aceptados y 2,763
    sinónimos oficiales) con acceso hash O(1).
  - Se incorporaron pruebas unitarias completas en
    `tests/testthat/test-matching.R` alcanzando 72 pruebas exitosas en
    el paquete (`0 FAIL | 0 WARN | 72 PASS`).
- **Mensaje de inicio e interactividad estilo tidyverse (`R/zzz.R`):**
  - Se implementó el hook `.onAttach()` con la estética visual y diseño
    de reglas de [cli](https://cli.r-lib.org) al estilo
    [tidyverse](https://tidyverse.tidyverse.org): encabezado con versión
    (destacando sufijos `.9000` en rojo), cuadrícula en dos columnas con
    marcas de verificación (`tick`) para los 4 listados oficiales y el
